...
## 设置定时自动运行（Linux crontab）

使用 crontab 可以让脚本定期自动执行。以下以 `reannounce.py` 每 30 分钟运行一次为例，`auto_delete.py` 配置方式相同。
建议先手动执行，测试没有问题再设置自动任务

**1. 确认脚本路径和 Python 路径**

```bash
realpath reannounce.py # 查询脚本绝对路径
```
...
在打开的编辑器中添加以下内容（按实际路径修改）：

```crontab
# 每 30 分钟执行一次强制汇报
*/30 * * * * python3 /root/AutoReannounce/reannounce.py

# 每 30 分钟执行一次自动删除
*/30 * * * * python3 /root/AutoReannounce/auto_delete.py
```

> `>> xxx.log 2>&1` 表示将输出追加写入日志文件，方便排查问题，可以加上。
